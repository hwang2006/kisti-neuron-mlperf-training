...
def get_pretrain(
    ...
    if disable_ce_fusion:
        llama31_config.cross_entropy_loss_fusion = False

    if enable_recompute:
        llama31_config.recompute_granularity = "full"
        llama31_config.recompute_method = "block"
        llama31_config.recompute_num_layers = 1
    pretrain.model.config = llama31_config

    pretrain.trainer.strategy.tensor_model_parallel_size = tensor_parallel_size
    ...
    precision = run.Config(
        ...
        fp8=None if disable_fp8 else "hybrid",
        fp8_amax_history_len=4,
        fp8_amax_compute_algo='most_recent',
        fp8_params=False if disable_fp8 else True,
...
def get_data(
    ...
    if use_last_256_shards:
        # C4 raw shards 768-1023, represented by merged/preprocessed en_6 + en_7.
        train_datasets = sum(
...
def get_parser() -> argparse.ArgumentParser:
    ...
    model_group.add_argument("--disable_fp8", action="store_true", help="Disable FP8 and use BF16 mixed precision")
    model_group.add_argument("--enable_recompute", action="store_true", help="Enable activation recomputation")
    model_group.add_argument("--disable_ce_fusion", action="store_true", help="Disable fused cross entropy loss")
...
if __name__ == "__main__":
    ...
    exp_prefix, pretrain = get_pretrain(
        max_lr=args.max_lr,
        disable_fp8=args.disable_fp8,
        enable_recompute=args.enable_recompute,
        disable_ce_fusion=args.disable_ce_fusion,
        size=args.size,
        nnodes=args.nodes,
