...
if [ "${ENABLE_RECOMPUTE:-0}" -gt 0 ]; then
    CMD_SUFFIX="${CMD_SUFFIX} --enable_recompute"
fi

if [ "${DISABLE_CE_FUSION:-0}" -gt 0 ]; then
    CMD_SUFFIX="${CMD_SUFFIX} --disable_ce_fusion"
fi

python3 pretrain_llama31_a100.py \
--user $USER --host $HOST \
--job_dir $JOB_DIR \
